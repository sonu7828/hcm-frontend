...
const LeaveApproval = () => {
   ...
   const availableMembers = useMemo(() => {
      return (teamMembers && teamMembers.length > 0) ? teamMembers : defaultMembers;
   ...
   const [newIncrement, setNewIncrement] = useState({ employeeId: '', requestedSalary: '', effectiveDate: '', reason: '' });
   ...
   const filteredRequests = useMemo(() => {
      return leaveRequests.filter(r => {
         ...
         if (matchStatus === 'Manager_approved') matchStatus = 'Approved';
   ...
   const handleStatusUpdate = async (id, status) => {
      try {
         ...
         // updateLeaveStatus will handle showing its own error toast, but we can clean up UI state if needed
   ...
   const handleIncrementRequest = async (e) => {
      e.preventDefault();
      const emp = teamMembers.find(m => m.id === newIncrement.employeeId);
      if (!emp || !newIncrement.requestedSalary || !newIncrement.effectiveDate || !newIncrement.reason) {
         showToast('Please fill in all required fields.', 'error');
         return;
      }
      await requestSalaryIncrement({
         employeeId: newIncrement.employeeId,
      ...
      setNewIncrement({ employeeId: '', requestedSalary: '', effectiveDate: '', reason: '' });
   };
   ...
   return (
      <div className="space-y-8 pb-12 animate-fade-in relative">
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
               <h1 className="hcm-page-title">Team Approvals</h1>
               <p className="hcm-page-subtitle">Review, manage and approve your team's leave and increment requests</p>
            </div>
            <div className="flex items-center gap-3">
               ...
               {activeModule === 'increments' && (
                  <button onClick={() => setShowIncrementModal(true)} className="btn-primary flex items-center gap-2 shadow-lg shadow-primary-500/20">
                     <Plus size={18} />
                        ...
                        {filteredRequests.map((req) => (
                           <tr key={req.id} className="hcm-tr">
                              ...
                              <td className="hcm-td text-right">
                                 {(req.status === 'PENDING' || req.status === 'Pending') && req.canApprove === true ? (
                                    ...
                                    <span className={cn(
                                       ...
                                       req.status === 'Approved' ? "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-450 border-emerald-100 dark:border-emerald-900/30" :
                                          req.status === 'MANAGER_APPROVED' || req.status === 'Manager_approved' ? "bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-900/30" :
                                 ...
                                 <td className="hcm-td text-right">
                                    {req.status === 'Pending' ? (
                                       <div className="flex justify-end items-center gap-2">
                                          ...
                                          >Approve</button>
                                          <button
                                             onClick={() => reviewIncrement(req.id, 'Rejected')}
            ...
            {selectedRequest && (
               <div className="p-6 sm:p-8 space-y-6 text-left bg-white dark:bg-slate-900">
                  ...
                  <div className="pt-6 border-t border-slate-50 dark:border-slate-800 flex items-center gap-3">
                     {selectedRequest.canApprove === true ? (
                        <>
                           <button
                           ...
                           <button
                              onClick={() => handleStatusUpdate(selectedRequest.id, 'MANAGER_APPROVED')}
                        ...
                        <div className="w-full text-center text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-xs py-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
         ...
         >
            <form onSubmit={handleIncrementRequest} className="p-6 sm:p-8 space-y-4 sm:space-y-6 text-left bg-white dark:bg-slate-900">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 text-left">
                  <div className="space-y-2 text-left">
                     <label className="form-label text-[10px] uppercase tracking-widest mb-1.5 block">Requested Salary (Monthly)</label>
                     <input
                        type="number"
                     ...
                     {newIncrement.employeeId && (
                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">
                           Current: {formatCurrency(teamMembers.find(m => m.id === newIncrement.employeeId)?.monthlyCTC || 0)} / mo
                        </p>
                     )}
                  </div>
                  <div className="space-y-2 text-left">
               ...
               <div className="pt-4 flex flex-col gap-3 text-left">
                  <button type="submit" className="btn-primary w-full py-2.5 sm:py-3 font-bold uppercase tracking-[0.2em] shadow-md shadow-primary-500/20 text-sm">Submit Request</button>
                  <button type="button" onClick={() => setShowIncrementModal(false)} className="w-full py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">Discard Request</button>
               </div>
            </form>
         </CenterModal>
      </div>
